#=##############################################################################
# NOTE:
    This code has to be used in the file "src/FLOWUnsteady_rotor.jl" 
...
* `aoa_bound_min`::Array{Float64,1}    : minimum angle of attack boundaries at defined airfoil position boundaries (positions must be the same as in the airfoils file!!!)
* `aoa_bound_max`::Array{Float64,1}    : maximum angle of attack boundaries at defined airfoil position boundaries (positions must be the same as in the airfoils file!!!)
...
function generate_rotor(Rtip::Real, Rhub::Real, B::Int,
                        ...
                        read_polar=vlm.ap.read_polar,
                        aoa_bounds::Array{Float64,2}=Array{Float64, 2}(undef, 0, 0),
                        # PROCESSING OPTIONS
                        pitch=0.0,
    ...
    #println("Generation Rotor with theta =")
    #println(_spl_theta)
    ...
    if length(aoa_bounds) == 0
        aoa_bound_min = Array{Float64, 1}(undef, 0)
        aoa_bound_max = Array{Float64, 1}(undef, 0)
    elseif length(aoa_bounds[:,1]) != length(airfoils)
        error("It seems that the given angle of attack boundaries are not related to te airfoil positions. 
                Please provide for each airfoil on each airfoil position the according angle of attack boundaries as input!")
    else 
        aoa_bound_min = aoa_bounds[:,2]
        aoa_bound_max = aoa_bounds[:,3]
    end
    propeller = vlm.Rotor(CW, r, chord, theta, LE_x, LE_z, B, airfoils, turbine_flag, aoa_bound_min, aoa_bound_max)
