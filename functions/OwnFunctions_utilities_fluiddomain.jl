#=##############################################################################
# DESCRIPTION
    NOTE: This file is based on the file "FLOWVPM/examples/utilities/utilities_fluiddomain" file created by Eduardo J. Alvarez
    ...
    => CHANGES: The functions are also able to calculate the wake velocity profiles for cylindrical 2D planes 
    axially located behind the turbine in x direction.
...
function computefluiddomain(pfield::vpm.ParticleField,
                                    grids::Array{<:uns.gt.AbstractGrid};
                                    spacetransform=nothing,     # only used to calculate velocity profiles if a cylindrical grid is calculated
                                    radial_pos=nothing,         # only used to calculate velocity profiles if a cylindrical grid is calculated
                                    x_loc=nothing,              # only used to calculate velocity profiles if a cylindrical grid is calculated
                                    # PROCESSING OPTIONS
                                    add_J=false,
                                    add_Uinf=false,
                                    add_Wapprox=false,
        ...
        for field_name in fields
            for grid in grids
                if !(field_name in keys(grid.field))
                    ...
                    uns.gt.add_field(grid, field_name, "vector", arr, "node")
                end
            end
        end
    end
    ...
    t = @elapsed begin
        ...
        for (grid, gridname) in zip(grids, _grid_names)
            ...
            if spacetransform == uns.gt.cylindrical3D
                nnodes = grid.nnodes
                num_radial_pos = grid.NDIVS[1] + 1
                #println("node number = $(nnodes)")
                #println("num_radial_pos = $(num_radial_pos)")

                # calculate the averaged velocities of Ux on each radial station
                Ux_avg_distr = zeros(num_radial_pos)
                for r = 1:num_radial_pos
                    avg = 0
                    for i = r:num_radial_pos:nnodes 
                        avg = avg + (grid.field["U"]["field_data"][1,i] / (nnodes/num_radial_pos))
                    end
                    Ux_avg_distr[r] = avg
                end

                # calculate the averaged velocities of Uy on each radial station
                Uy_avg_distr = zeros(num_radial_pos)
                for r = 1:num_radial_pos
                    avg = 0
                    for i = r:num_radial_pos:nnodes 
                        avg = avg + (grid.field["U"]["field_data"][2,i] / (nnodes/num_radial_pos))
                    end
                    Uy_avg_distr[r] = avg
                end

                # calculate the averaged velocities of Uz on each radial station
                Uz_avg_distr = zeros(num_radial_pos)
                for r = 1:num_radial_pos
                    avg = 0
                    for i = r:num_radial_pos:nnodes 
                        avg = avg + (grid.field["U"]["field_data"][3,i] / (nnodes/num_radial_pos))
                    end
                    Uz_avg_distr[r] = avg
                end

                # calculate the averaged velocities of the whole plane based on radial averages 
                Ux_avg = 0
                for r = 1:num_radial_pos 
                    Ux_avg = Ux_avg + (Ux_avg_distr[r] / num_radial_pos)
                end

                #Ux_avg = 0
                #for i = 1:nnodes 
                #    Ux_avg = Ux_avg + (grid.field["U"]["field_data"][1,i] /(nnodes)
                #end

                Uy_avg = 0
                for r = 1:num_radial_pos 
                    Uy_avg = Uy_avg + (Uy_avg_distr[r] / num_radial_pos)
                end

                Uz_avg = 0
                for r = 1:num_radial_pos 
                    Uz_avg = Uz_avg + (Uz_avg_distr[r] / num_radial_pos)
                end
                
                #println("avg ges Ux= $(Ux_avg)")
                #println("avg ges Uy= $(Uy_avg)")
                #println("avg ges Uz= $(Uz_avg)")
                #println("radial_pos= $(radial_pos)")
                #println("avg distr Ux= $(Ux_avg_distr)")
                #println("avg distr Uy= $(Uy_avg_distr)")
                ...
                if save_path != nothing
                    ...
                    Uz_avg_vec = zeros(num_radial_pos)
                    Ux_avg_vec .= Ux_avg
                    Uy_avg_vec .= Uy_avg
                    Uz_avg_vec .= Uz_avg

                    data_matrix = hcat(radial_pos, 
                                        Ux_avg_vec, Uy_avg_vec, Uz_avg_vec, 
                                        Ux_avg_distr, Uy_avg_distr, Uz_avg_distr)               # matrix with all data
                    df = DataFrame(data_matrix, :auto)                                          # matrix converted into dataframe

                    CSV.write(fname, df, header=["r(m)", 
                                                 "Ux_avg(m/s)", "Uy_avg(m/s)", "Uz_avg(m/s)",
                                                 "Ux_avg_distr(m/s)", "Uy_avg_distr(m/s)", "Uy_avg_distr(m/s)"])                 # Create and save csv file
                    
                    #header = ["r(m)" "Ux_avg(m/s)" "Uy_avg(m/s)" "Uz_avg(m/s)" "Ux_avg_distr(m/s)" "Uy_avg_distr(m/s)" "Uy_avg_distr(m/s)"]
                    #CSV.write(fname, vcat(header, data_matrix))                                                      # Create and save csv file
            ...
            if save_path != nothing
                str *= uns.gt.save(grid, file_pref*gridname;
...
function computefluiddomain(pfield::vpm.ParticleField,
    ...
    for (numi, num) in enumerate(nums)
        ...
        if origin!=nothing || orientation!=nothing
            ...
            for grid in grids
                uns.gt.lintransform!(grid, M, T; reset_fields=true)
            end
...
function computefluiddomain(P_min, P_max, NDIVS, args...;
    ...
    grid = uns.gt.Grid(P_min, P_max, NDIVS; grid_optargs...)
    
    # save/calculate the positions of the cylindrical grid
    if spacetransform == uns.gt.cylindrical3D
        radial_pos = zeros(NDIVS[1] + 1)
        for i = 1:NDIVS[1] + 1
            radial_pos[i] = grid.nodes[1,i]
        end
    ...
    if spacetransform!=nothing
        uns.gt.transform!(grid, spacetransform)
    end

    # Translate and rotate the field to the given origin and orientation
    uns.gt.lintransform!(grid, Oaxis, O)
    ...
    if debug && save_path!=nothing
        ...
        str = save_path
        str *= uns.gt.save(grid, "fdom-debug"; path=save_path)
