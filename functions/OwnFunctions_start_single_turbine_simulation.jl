...
function start_single_turbine_simulation(# ---- ESSENTIAL ARGUMENTS ---------
  ...
  J               = magVinf/((RPM/60)*2*R)                                    # Advance ratio Vinf/(nD)
  magVinfx        = magVinf*cosd(AOA)                                         # wind velocity in x direction
  Vinf(X, t)      = magVinf*[cosd(AOA), sind(AOA), 0]                         # Freestream velocity vector (m/s) 
  ...
  const_solution  = VehicleType==uns.QVLMVehicle                              # Whether to assume that the solution is constant or not

  # Time parameters
  nsteps          = const_solution ? 2 : nrevs*nsteps_per_rev                 # Number of time steps
  ttot            = nsteps/nsteps_per_rev / (RPM/60)                          # Total simulation time (s) 
  ...
  sigma_vpm_overwrite = lambda_vpm * 2*pi*R/(nsteps_per_rev*p_per_step)       # VPM smoothing radius

  # Rotor solver
  vlm_rlx         = 0.5                                                       # VLM relaxation <-- this also applied to rotors
  hubtiploss_correction = vlm.hubtiploss_correction_prandtl                   # Hub and tip loss correction 
                                                                              # (options: vlm.hubtiploss_nocorrection, 
  ...
  # ----------------- WAKE TREATMENT ---------------------------------------------
  # !!!!! THIS WILL DO NOTHING RIGHT NOW. 
  # !!!!! IF suppress_fountain = true, 
  # !!!!! PARTICLES ARE NOT SHED DURING THE FIRST REVOLUTIONS IN THE HUB AREA TO AVOID FOLLOWING PROBLEM:
  # !!!!!
  ...
  if fidelity == "low"
      suppress_fountain   = false#true                                            # Toggle
  elseif fidelity == "mid"
      suppress_fountain   = false#true
  elseif fidelity == "high"
  ...
  if postprocessing
    ...
    fdom_suffixes = single_turbine_simulation_postprocessing(
